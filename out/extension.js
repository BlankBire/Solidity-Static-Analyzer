...
function runAnalysis(document) {
  ...
  const naming = {
    functionPattern: config.get(
      "naming.functionPattern",
      "^[A-Za-z_][A-Za-z0-9_]*$"
    ),
    variablePattern: config.get(
      "naming.variablePattern",
      "^[A-Za-z_][A-Za-z0-9_]*$"
    ),
    constantPattern: config.get(
      "naming.constantPattern",
      "^[A-Za-z_][A-Za-z0-9_]*$"
    ),
    contractPattern: config.get(
  ...
  const payableNamePattern = config.get(
    "payableNamePattern",
    "deposit|buy|mint|stake|fund|contribute|donate|tip|payIn|addLiquidity"
  );
  const findings = (0, solidityAnalyzer_1.analyzeText)(
    text,
    rules,
    maxProblems,
    naming,
