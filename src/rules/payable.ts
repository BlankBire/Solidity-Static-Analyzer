...
export interface PayableHeuristicConfig {
  enabled: boolean;
  pattern: string;
}
...
export function runPayableAstAndHeuristic(
  ...
  tree: any | undefined,
  enabled: boolean,
  heuristic: PayableHeuristicConfig | undefined,
  ...
  if (tree) {
    try {
      ...
      const walkFunctions = (node: any) => {
        if (!node) return;
        const t = String(node.type);
        const isStdFunction =
          t === "function_definition" || t === "function_declaration";
        ...
        if (isStdFunction || isReceiveNode || isFallbackNode) {
          const bodyNode = (node.namedChildren || node.children || []).find(
            (c: any) =>
              String(c.type) === "function_body" || String(c.type) === "block"
          );
          ...
          if (shouldWarn) {
            ...
            if (isReceive) tryMark("receive");
            else if (isFallback) tryMark("fallback");
            else if (isConstructor) tryMark("constructor");
            else {
              const fm = headerText.match(
                /\bfunction\b\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(/
              );
        ...
        for (const c of kids) walkFunctions(c);
      };
      if (root) walkFunctions(root);
    } catch {}
  }
 ...
 */
export function runPayableLineFallbackSingle(
  line: string,
  lineIndex: number,
  content: string,
  tree: any | undefined,
